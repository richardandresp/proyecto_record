...
// Total
$sqlCount = "SELECT COUNT(*) FROM zona z $where_sql";
...
// Datos
$sql = "SELECT z.id, z.nombre, z.activo
        FROM zona z
        $where_sql
        ORDER BY z.nombre
        LIMIT ? OFFSET ?";
...
function build_url($page, $per_page) {
  ...
  return BASE_URL . '/admin/zonas.php?' . http_build_query($qs);
}
...
<div class="container">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h3>Zonas</h3>
    <a class="btn btn-primary" href="<?= BASE_URL ?>/admin/zona_edit.php">+ Nueva zona</a>
  </div>

  <form class="row g-2 mb-3" method="get">
    <div class="col-md-4">
      <input class="form-control" name="q" placeholder="Buscar por nombre..." value="<?= htmlspecialchars($q) ?>">
    </div>
    <div class="col-md-3">
    ...
    <div class="col-md-5 d-flex gap-2">
      ...
      <a class="btn btn-secondary" href="<?= BASE_URL ?>/admin/zonas.php">Limpiar</a>
    </div>
  </form>
  ...
  <div class="table-responsive">
    <table class="table table-sm table-striped align-middle">
      ...
      <tbody>
      <?php if (!$rows): ?>
        <tr><td colspan="4" class="text-center py-4">Sin resultados</td></tr>
      <?php else: ?>
        <?php foreach($rows as $r): ?>
          <tr>
            ...
            <td>
              <?= ((int)$r['activo']===1) ? '<span class="badge bg-success">Activa</span>' : '<span class="badge bg-danger">Inactiva</span>' ?>
            </td>
            <td class="d-flex gap-2">
              <a class="btn btn-sm btn-outline-secondary" href="<?= BASE_URL ?>/admin/zona_edit.php?id=<?= (int)$r['id'] ?>">Editar</a>
              <?php if ((int)$r['activo']===1): ?>
                <a class="btn btn-sm btn-outline-danger"
                   href="<?= BASE_URL ?>/admin/zona_toggle.php?id=<?= (int)$r['id'] ?>&to=0"
                   data-confirm="¿Inactivar esta zona?" data-confirm-type="warning" data-confirm-ok="Sí, inactivar">Inactivar</a>
              <?php else: ?>
                <a class="btn btn-sm btn-outline-success"
                   href="<?= BASE_URL ?>/admin/zona_toggle.php?id=<?= (int)$r['id'] ?>&to=1"
                   data-confirm="¿Activar esta zona?" data-confirm-type="info" data-confirm-ok="Sí, activar">Activar</a>
              <?php endif; ?>
            </td>
          </tr>
        <?php endforeach; ?>
